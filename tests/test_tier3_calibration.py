...
V2_FIXTURE_PATH = ROOT / "data" / "tier3_calibration_pairs_v2.csv"
...
class Tier3CalibrationTests(unittest.TestCase):
    ...
    def test_every_unique_text_is_encoded_once_through_pattern_encoder(self):
        for fixture in (FIXTURE_PATH, V2_FIXTURE_PATH):
            with self.subTest(fixture=fixture.name):
                ...
                scored, unique_text_count = score_pairs(
                    pairs, PatternEncoder(embedder)
                )
                ...
                self.assertTrue(
                    all(-1 <= float(row["score"]) <= 1 for row in scored)
                )
    ...
    def test_committed_real_reports_match_fixtures_and_runner(self):
        runner_path = ROOT / "src" / "calibrate_tier3.py"
        cases = (
            (
                FIXTURE_PATH,
                ROOT / "docs" / "tier3_calibration_results_v1.json",
                5,
            ),
            (
                V2_FIXTURE_PATH,
                ROOT / "docs" / "tier3_calibration_results_v2.json",
                4,
            ),
        )

        for fixture_path, report_path, expected_overlaps in cases:
